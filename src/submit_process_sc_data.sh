...
d=MONDO:0005086
echo "python process_sc_data.py ${d} --output_dir ${outdir} --keep_all_genes True"  | bsub -G teichlab -o logfile-pbulk-%J.out -e logfile-pbulk-%J.err -M100000 -R "select[mem>100000] rusage[mem=100000]
...
for d in $(cat failed_mondos.txt); do \
    echo "python process_sc_data.py ${d} --output_dir ${outdir} --keep_all_genes True"  | \
        bsub -G teichlab -o logfile-failedpbulk-%J.out -e logfile-failedpbulk-%J.err -M300000 -R "select[mem>300000] rusage[mem=300000]" 
done 
...
for d in $disease_ids; do \
# for d in $(cat failed_mondos.txt); do \
# for d in MONDO:0004298 MONDO:0001056 MONDO:0004966; do \
    d2=$(echo $d | sed 's/:/_/')
    if [ ! -f ${outdir}/cellxgene_targets_${d2}.pbulk_all_genes.h5ad ]; then
        echo "${d} missing"
    else 
...
# de_ready_disease_ids=$(ls $outdir/cellxgene_*pbulk_all_genes.h5ad | cut -f 9 -d '/' | sed 's/cellxgene_targets_//' | sed 's/.pbulk_all_genes.h5ad//')
de_ready_disease_ids=$(cat /nfs/team205/ed6/bin/sc_target_evidence/src/de_ready_mondo.txt)
